...
| 2026-08-09 | **Sprint 5c complete — `v0.4.0`, the committed finish line.** Chaos suite (5 real-kill scenarios + Toxiproxy fault injection), README as hiring artifact, testing strategy, `wassal.sh` control script, demo script. Recovery measured at **6.1 s** against a 30 s target | Committed scope reached |
| 2026-08-09 | **Sprint 4 complete.** Simulator on the real road graph, hot/cold location split, cross-instance WebSocket fan-out confirmed on both gateways, observability tier. **NFR-003 clarified and restated** — 494× reduction in write statements, not rows | Exit criteria met |
