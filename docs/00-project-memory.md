...
|---|------------|-----------------|---------------------|
| A-01 | `[AUTO]` Simulator movement uses a **precomputed OSM road graph** (offline extract → node/edge asset, random walk) rather than a runtime routing engine. *Rationale: identical fidelity for load generation at ~20 h less cost and one fewer container* | If true OD routing is wanted, Sprint 4 grows by ~15–20 h and gains an OSRM/GraphHopper dependency | Phase 12 |
