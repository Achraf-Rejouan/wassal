...
public class DispatchOutboxPublisher {
    ...
    public DispatchOutboxPublisher(
    ...
    public void publishPending() {
        ...
        for (Map<String, Object> row : pending) {
            ...
            jdbc.update(
                    "UPDATE dispatch.dispatch_outbox SET sent_at = now(), attempts = attempts + 1"
                            + " WHERE id = :id",
                    new MapSqlParameterSource("id", id));

            Instant createdAt = ((java.sql.Timestamp) row.get("created_at")).toInstant();
            metrics.timer("wassal_dispatch_outbox_publish_lag")
    ...
    public void reportDepth() {
        Integer depth =
                jdbc.queryForObject(
                        "SELECT count(*) FROM dispatch.dispatch_outbox WHERE sent_at IS NULL",
                        new MapSqlParameterSource(),
                        Integer.class);
        metrics.gauge("wassal_dispatch_outbox_depth", depth == null ? 0 : depth);
    }
}
